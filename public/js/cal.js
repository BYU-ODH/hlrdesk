  ...
  function displayWindow() {
    ...
    if (firstCell.hasClass('booked')) {
      ...
      for (var i = 0; i < currentEvents.length; i++) {
        if (currentEvents[i].id == firstCell.data('event')) {
          ...
          try {
            noteObj = JSON.parse(cellEvent['notes']);
          } catch (e) {
            noteObj = {};
          }
          break;
      ...
      $('#clientNetid').text(noteObj.name ? noteObj.name : cellEvent.contact_netid);
      $('#proxyEvent').hide();
      if (cellEvent.contact_netid !== cellEvent.responsible_netid) {
        $('#proxyEvent').show();
        $('#overseerNetid').text(noteObj.overseerName ? noteObj.overseerName : cellEvent.responsible_netid);
      }
      $('#contactEmail').text(noteObj.email);
      $('#contactPhone').text(noteObj.phone);
    ...
    $('#saveEvent').click(function() {
      ...
      if (currentView == 'multiple rooms') {
        ...
        var date = displayedDate;
        date.add(days[cellColumn], 'days');
        var start_date = date.format('YYYY')+','+date.format('M')+','+date.format('D')+','+Math.floor(firstCell.data('time'))+','+(firstCell.data('time')%1===0?'0':'30')+',0,'+(Number(date.format('d'))+1);
      }
    ...
    $('#editEvent').click(function() {
      ...
      editedEvent['note'] = cellEvent['notes'];
...
/////////////////////////////////////////////////BACKEND/////////////////////////////////////////
  ...
  socket.on('refresh events', function() {
    ...
    if (currentView == 'multiple rooms') {
      ...
      var monday = displayedDate;
      monday.day(1);
      getRoomEventsForWeek(window.rooms[roomId], monday);
    }
  });
